...
| `POST /v2/pay/join.verify` | 付费进群入账验证（公开 Base RPC 解析 Transfer 日志）；校验 tx 发送方 == 申请人声明的 payer，并验证钱包密钥收据 | ❌ |
| `POST /v2/pay/join.receipt` | 铸造钱包密钥收据（对 {fp, tx_hash, payer} 用钱包密钥 ES256 签名，证明付款钱包归申请人所有——付费进群防重放） | ✅ |
...
```sh
# 单元测试（RLP 向量 / JWT 结构 / keccak256 向量 / 链上日志解析 / 金额换算 / 收据验签）——离线可跑
go test ./payment/...
...
│                         EIP-1559 RLP（USDC ERC-20 transfer）、keccak256/地址派生、
│                         钱包密钥签名（收据）
├── internal/rpcclient/   公开 Base RPC：交易收据（含 from）、Transfer 日志解析、余额、gas
├── internal/payapi/      /v2/pay/* HTTP 层 + A2A 验签中间件 + 收据验证
